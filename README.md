---
title: Semantic Search with Embeddings and FAISS
emoji: 🔎
colorFrom: blue
colorTo: gray
sdk: docker
...
The app expects all generated search artifacts and the cached embedding model under `indexes/`.
If they are missing, run `python -m src.build_index` first.
...
```bash
pytest -q
make test  # optional on systems with make
```
...
No secrets are required. Optional environment variables are documented in `.env.example`:

- `MODEL_NAME`: SentenceTransformers model name.
- `DEFAULT_TOP_K`: default result count in the UI.
- `PORT`: Streamlit port for Docker/hosted deployments.

Only use `indexes/tfidf.joblib` artifacts generated by this project; `joblib` files are not safe to load from untrusted sources.
...
```bash
docker build -t semantic-search-faiss .
docker run --rm -p 8501:8501 semantic-search-faiss
```
...
The Docker build installs dependencies, builds retrieval artifacts, evaluates retrieval, and starts Streamlit on port `8501` by default. A first build needs network access to download Python packages and the embedding model. Persist `indexes/` only if you want to avoid rebuilding artifacts between deployments.
